...
export async function GET(request: Request) {
  try {
    ...
    console.log('[v0] Query result:', { credentials, error, count: credentials?.length })
    ...
    if (!credentials || credentials.length === 0) {
      ...
      return NextResponse.json({ error: 'No password found for this user. Please set a new password.' }, { status: 404 })
    }

    // Return the password (stored in plain text in password_hash field for admin)
    return NextResponse.json({
      password: credentials[0].password_hash || 'No password set',
    })
  } catch (error: any) {
